...
void GOLScene::Update()
{
	...
	if (m_input.GetKeyDown(SDL_SCANCODE_SPACE))
	{
		...
		{
			nextCells->m_data[i] = (random(100) == 0) ? 1 : 0;
		}
	}
	...
	for (int y = 1; y < nextCells->m_height - 1; y++)
	{
		for (int x = 1; x < nextCells->m_width - 1; x++)
		{
			...
			count += currentCells->Read(x, y - 1);		// upper middle
			count += currentCells->Read(x + 1, y - 1);	// upper right
			count += currentCells->Read(x - 1, y);		// middle left
			count += currentCells->Read(x + 1, y);		// middle right
			count += currentCells->Read(x - 1, y + 1);	// lower left
			count += currentCells->Read(x, y + 1);		// lower middle
			count += currentCells->Read(x + 1, y + 1);	// lower right
